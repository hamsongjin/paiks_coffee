      ...
      {item.options.map((option) => (
        ...
        >
          <span className="min-w-0">
            <span className="mr-1 text-neutral-400">{option.opt_type}</span>
            <span className="truncate">{option.opt_nm}</span>
            {option.count > 1 ? <span className="ml-1">x {option.count}</span> : null}
          </span>
          <span className="shrink-0">+{formatPrice(option.opt_price * option.count)}원</span>
        </li>
      ))}
    </ul>
  );
...
export function CartList() {
  ...
  if (items.length === 0) {
    return (
      <section className="flex flex-1 flex-col items-center justify-center px-4 py-16 text-center">
        ...
        <p className="mt-2 text-sm text-neutral-500">메뉴에서 상품과 옵션을 선택해 담아주세요.</p>
        <Link
          href="/menu"
          className="mt-5 rounded-md bg-neutral-950 px-4 py-3 text-sm font-semibold text-white"
        >
          메뉴 보기
        </Link>
      </section>
  ...
  return (
    <>
          ...
          <article key={item.id} className="px-4 py-4">
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-neutral-200 text-base font-semibold text-neutral-950"
                  onClick={() => decreaseItem(item.id)}
                >
                  -
                </button>
                <span className="w-8 text-center text-sm font-semibold text-neutral-950">
                  {item.quantity}
                </span>
                <button
                  type="button"
      ...
      <footer className="sticky bottom-0 border-t border-neutral-100 bg-white p-4">
        <div className="flex items-center justify-between gap-4">
          <span className="text-sm font-semibold text-neutral-500">총 금액</span>
          <span className="text-xl font-bold text-neutral-950">{formatPrice(totalPrice)}원</span>
        </div>
        {errorMessage ? (
        ...
        <button
          type="button"
          className="mt-3 w-full rounded-md bg-neutral-950 px-4 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-neutral-300"
          disabled={isPending}
          onClick={submitOrder}
        >
